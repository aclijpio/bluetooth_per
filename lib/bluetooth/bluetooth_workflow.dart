...
class BluetoothWorkflow {
  ...
  Future<Either<Failure, List<BluetoothDevice>>> _scanForDevices() async {
    try {
      ...
      // Сначала добавляем уже спаренные устройства
      try {
        print('🔍 [BluetoothWorkflow] Проверяем спаренные устройства...');
        ...
        for (final d in bonded) {
          if (d.name != null &&
            ...
            if (!devices.any((e) => e.address == d.address)) {
              ...
              seenDevices.add(d.address);
              print(
                  '➕ [BluetoothWorkflow] Добавлено спаренное устройство: ${d.name} (${d.address})');
            }
          }
        }
        print('ℹ️ [BluetoothWorkflow] Спаренных устройств: ${bonded.length}');
      } catch (e) {
        print(
            '⚠️ [BluetoothWorkflow] Не удалось получить спаренные устройства: $e');
      }
      ...
      print('🔍 [BluetoothWorkflow] Запускаем активное сканирование');
      _flutterBlueClassic.startScan();

      // safety-таймаут 25 секунд
      Timer(const Duration(seconds: 25), () async {
        if (!completer.isCompleted) {
          print('⏰ [BluetoothWorkflow] Таймаут сканирования (25 секунд)');
  ...
  Future<Uint8List> _downloadArchiveData() async {
    ...
    final subscription = _transport.bytes.listen(
      (data) {
        ...
        if (!headerSkipped) {
          if (chunk.length >= 4) {
            chunk = chunk.sublist(4);
            headerSkipped = true;
          } else {
            buffer.addAll(chunk);
            if (buffer.length >= 4) {
              final remaining = buffer.sublist(4);
              buffer
                ..clear()
                ..addAll(remaining);
              headerSkipped = true;
        ...
        print(
            '📨 [BluetoothWorkflow] Получен блок данных: ${chunk.length} байт');
        buffer.addAll(chunk);
        print(
